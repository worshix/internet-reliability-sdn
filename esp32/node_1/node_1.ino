...
//  TIMING
// ================================================================
#define PING_INTERVAL_MS      2000   // send pings this often (per peer)
#define TELEMETRY_INTERVAL_MS 5000   // flush stats to gateway this often
#define PONG_TIMEOUT_MS        500   // consider ping lost after this
...
typedef struct __attribute__((packed)) {
    ...
    uint8_t  src_node;
    uint16_t ping_id;
    uint32_t sent_ms;     // original send time echoed back
} PongPkt;                // 8 bytes
...
struct PeerStats {
    ...
    // accumulators — reset after each telemetry flush
...
void onDataSent(const uint8_t *mac, esp_now_send_status_t status) {
    // no-op for now; could track send failures here
}
...
void onDataRecv(const esp_now_recv_info_t *recv_info,
    ...
    case PKT_PONG: {
        ...
        if (p.ping_pending && p.pending_ping_id == pong->ping_id) {
            ...
            p.ping_pending = false;
            p.pongs_received++;
            p.latency_sum_ms += rtt / 2.0f;   // one-way estimate
            p.rssi_sum  += rssi;
            p.rssi_count++;
        }
        break;
...
void sendPing(int idx) {
    ...
    // expire stale pending ping
    if (p.ping_pending &&
        millis() - p.pending_sent_ms > PONG_TIMEOUT_MS) {
        p.ping_pending = false;
    }
    if (p.ping_pending) return;   // still waiting for previous pong
...
void setup() {
    ...
    // register all peers with ESP-NOW
